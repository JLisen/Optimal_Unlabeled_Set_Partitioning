...
mu=0.491#Transmission Rate
k_max=3#contrianed, maximum edges(subgroups) allowed <n-1
beta=1.02#economy 
...
while runs<10000:
    ...
    edge_list=[0]*99999999 ##define how many edges exactly later to save the running space
    ...
    #print(edge_list[1].startVertex.name) 
    #------------------
    ...
    #List for storing the number of items of each group 
    ...
    for i in range(0,V):
        ...
        if V==1:
            ...
            subset_cardi.append(V)
            
            #print("Threshold:",risk_vector[V-1])
            threshold_list.append(risk_vector[V-1])
            
        
            k=k+1
            i=i+1
    subset_N.append(subset_cardi)
    
    print("beta:",beta)
    print("Optimal Number Groups:", k)
    
    
    #Fig3
    N_groups.append(k)
    beta_groups.append(beta) 
